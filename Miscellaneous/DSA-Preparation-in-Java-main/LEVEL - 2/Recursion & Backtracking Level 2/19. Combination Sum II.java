...
class Solution {
    ...
    public void combination(List<Integer> ans,HashMap<Integer,Integer> freq,ArrayList<Integer> uniqueCoins,int currentCoin,int target)
    {
        if(target==0)
        {
...
ans.add(val);
                ...
                combination(ans,freq,uniqueCoins,currentCoin+1,target-i*val);
                for(int j=1;j<=i;j++)
                {
